...
## 현재 구현 스냅샷 (2026-03-30)

- 현재 코드에는 `Dashboard/Settings` 2탭, 문열기, 페어링 시작, WebSocket 로그 스트리밍, 계정 변경, WiFi 재설정, 수동 OTA 업로드가 있다.
- `현재 버전 표시`, `도어 설정 UI`, `기기 삭제/관리 UI`, `GitHub Releases 자동 OTA`는 아직 리포지토리에 구현되지 않았다. 태그 기반 릴리스 workflow는 이미 있다.
- 아래 문서 본문은 최종 제품 방향을 포함한다. 현재 구현과 구분이 필요한 항목은 `미구현`으로 명시한다.
...
| 설정 | 계정 변경, WiFi 재설정, 페어링 시작, 펌웨어 업데이트(수동 OTA 업로드) |

- 프론트엔드는 Vanilla HTML + JS 유지. 바이너리 임베드.
- SPA처럼 탭 전환은 JS로 처리 (페이지 리로드 없이).

**별도 상태 API 없음. 로그 스트림으로 현재 동작만 관찰한다.**

기기 목록/상태를 전용 API로 빼지 않는다. 현재 구현은 StateMachine(SM Task)과 Control Task가 찍는 `Feed`, `Unlock`, `GPIO HIGH/LOW` 로그를 브라우저가 WebSocket으로 읽는 방식이다.

SM Task 로그 예시:
```
[sm] Feed: AA:BB:CC:DD:EE:FF seen=yes
[sm] Unlock! Sending command to control task
[door] GPIO 4 HIGH
[door] GPIO 4 LOW — pulse complete
```
...
- WS 연결 이후 수신되는 로그를 전송한다. 연결 전 backlog 재생은 아직 없다.

### 5. GitHub Releases 자동 OTA

이 섹션은 **최종 제품 목표**이며, 현재 리포지토리에는 아직 구현되지 않았다.
...
- **CI/CD**: GitHub Actions로 `v*` 태그 push 시 호스트 테스트 + 펌웨어 빌드 + 릴리즈 생성을 수행한다. 태그에서 `v` 접두사를 제거한 정수를 `PROJECT_VER`로 주입 (`idf.py -DPROJECT_VER=N build`). 릴리즈에 `doorman.bin` + `doorman.elf` 첨부. ESP32 기기는 이후 자동 OTA 단계에서 이 릴리즈를 폴링한다. 일반 PR/커밋 CI는 아직 없다.
...
- **그 외 직접 호출, atomic 플래그, 공유 변수 최소화**
...
- `ota_updater.cpp` 신규 예정. GitHub API 폴링 + OTA 다운로드.

### 8. 완성 기준

1. 동료들 폰 등록 → 출근 시 자동 문열림 안정 동작
2. 웹에서 로그 스트림으로 현재 동작 확인 가능
...
- **기기 상태 = 로그로 해결** → 별도 상태 API 없음. SM/Control이 찍는 핵심 ESP_LOG를 WS로 스트리밍. 브라우저는 현재 동작만 본다.
- **문열림 정책?** → 감지 즉시. 나중에 RSSI 등 튜닝.
- **쿨다운?** → 미감지 전환 + 최소 시간 병행.
- **자동 OTA?** → potados99/doorman releases 폴링, firmware.bin, 정수 버전 비교.
- **웹 UI 구성?** → 현재 구현은 탭 분리 (대시보드, 설정). 기기관리 탭은 아직 없음.
...
- **CI/CD?** → 태그 푸시 시 호스트 테스트 + ESP-IDF 빌드 + GitHub Release를 수행하는 릴리스 workflow는 있다. 일반 PR/커밋 CI는 아직 없다.
