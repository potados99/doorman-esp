...
현재 리포지토리에서는 이 플랜의 핵심 범위가 구현 완료됐다. 추가로 `main/config_service.cpp`, `main/sm_task.cpp`, `main/bt_manager.cpp`까지 연결된 상태이며, 이 문서는 **3차 구현 당시의 계획 문서**로 읽어야 한다.

## Motivation

당시 Gatekeeper는 `feed()` 시그니처만 존재하고, 내부 로직이 없었다. 4차(BT 프로덕션화)에서 BT 스캔 결과를 연결하려면, 먼저 상태머신과 설정 구조체가 완성되어 있어야 했다. 호스트 테스트로 로직을 검증한 뒤에 하드웨어와 연결하는 순서가 맞았다.
...
- **Phase 4에서** `main/config_service.cpp`가 `getConfig()`/`setConfig()` with lock + NVS 로드/저장을 제공.
- 현재 구현은 SM 태스크 시작 시 AppConfig 스냅샷을 넘겨 StateMachine을 생성한다. 런타임 중 설정 변경의 live 반영은 아직 없다.
...
BLE는 수동 수신이라 advertisement가 안 들리면 콜백이 안 온다. 그래서 감지/미감지를 `seen` 불리언으로 명시하고, 시간 기반 전이는 `tick()`으로 분리한다:
- `feed(mac, seen, now_ms)`: 감지 이벤트 기록. `seen=false`면 즉시 미감지 전환.
- `tick(now_ms)`: 주기적 호출. 타임아웃에 의한 미감지 전환 + 쿨다운 만료 확인 + Unlock 판정

`ScanResult` enum은 제거하고, `bool seen`으로 BLE/Classic 호출 패턴을 통합한다.
...
- GitHub Actions 워크플로우: `.github/workflows/build.yml` (현재는 태그 기반 호스트 테스트 + ESP-IDF 빌드/릴리즈)
